...
Beta = matrix(rnorm(30*40), ncol=n, nrow=p)
dd1 = svd(err1)
...
Xnew = x1 + matrix(rnorm(n*p), ncol=p, nrow=n)[1:36,]
# Xnew = matrix(rnorm(n*p), ncol=p, nrow=n)[1:36,]
# X[,1] = X[,1] + rep(c(0,1), each=n/2)
order = sample(36)

Beta2 = matrix(rnorm(30*40), ncol=n, nrow=p)
test.x = Xnew[order,]
test.z = test.x %*% Beta2
test.z = test.z[1:36,1:36]
test.y = y1[order,]
...
# result = replicate(10,do.one(kernel=2))
...
write.csv(result, paste("simu.0618.kernel.compound", index, "sparse.csv", sep=""))
...
# autoplot(prcomp(tcrossprod(test.x)), colour=label) + geom_point()
# autoplot(prcomp(test.x%*%test.H%*%t(test.x)), colour=as.factor(test.y[,1])) + geom_point()
# colnames(test.y) = c("Ph-neg","Ph-pos")
# label = test.y[,1]
# label = ifelse(label==1, "neg", "pos")


# pca = prcomp(test.x)
# plot.data <- as.data.frame(pca$x)
# plot.data$label = label
# ggplot(plot.data) + aes(x=PC1, y=PC2, colour=label) + geom_point()
...
# plot.data <- as.data.frame(pca$x)
# plot.data$label = label
# ggplot(plot.data) + aes(x=PC1, y=PC2, colour=as.factor(y1[,1])) + geom_point()
...
# pca = prcomp(test.x %*% fit.os.old2$beta)
# plot.data2 <- as.data.frame(pca$x)
# plot.data2$label = label
# p2 = ggplot(plot.data2) + aes(x=PC1, y=PC2, colour=label) + geom_point()


# pca = prcomp(test.x)
# plot.data <- as.data.frame(pca$x)
# plot.data$label = label
# p1 = ggplot(plot.data) + aes(x=PC1, y=PC2, colour=label) + geom_point()
